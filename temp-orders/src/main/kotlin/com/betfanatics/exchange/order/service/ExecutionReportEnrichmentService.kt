    ...
    fun enrichExecutionReport(message: Message): ExecutionReport? {
        try {
            ...
            val enrichedReport = ExecutionReport(
                ...
                side = tradingSide,
                leavesQty = com.fbg.api.common.BetDecimal(leavesQty.toString()),
                cumQty = com.fbg.api.common.BetDecimal(cumQty.toString()),
                betOrderId = betOrderId ?: "UNKNOWN",
                newOrder = newOrder,
                modifyOrder = modifyOrder,
                cancelOrder = cancelOrder,
                instrument = instrument,
                lastQty = lastQty?.let { com.fbg.api.common.BetDecimal(it.toString()) },
                lastPx = lastPx?.let { com.fbg.api.common.BetDecimal(it.toString()) },
                clOrdID = clOrdId,
                origClOrdID = origClOrdId,
                orderQty = orderQty?.let { com.fbg.api.common.BetDecimal(it.toString()) },
                ordType = ordType,
                price = price?.let { com.fbg.api.common.BetDecimal(it.toString()) },
                timeInForce = tif,
                avgPx = avgPx?.let { com.fbg.api.common.BetDecimal(it.toString()) },
